								...
								if (dps) {
									...
									if (this.adapter.pendingRequests.has(id)) {
										...
										if (Array.isArray(result)) {
											result.forEach(dpsSingleResult => {
												if (typeof dpsSingleResult == "object" && dpsSingleResult.hasOwnProperty("in_cleaning")) {
													this.adapter.log.info("Unsolicited updated locally");
													this.adapter.setStateAsync(`Devices.${duid}.deviceStatus.in_cleaning`, dpsSingleResult.in_cleaning, true);
													this.adapter.setStateAsync(`Devices.${duid}.deviceStatus.in_returning`, dpsSingleResult.in_returning, true);
													this.adapter.setStateAsync(`Devices.${duid}.deviceStatus.in_fresh_state`, dpsSingleResult.in_fresh_state, true);
													this.adapter.setStateAsync(`Devices.${duid}.deviceStatus.lab_status`, dpsSingleResult.lab_status, true);
													this.adapter.setStateAsync(`Devices.${duid}.deviceStatus.state`, dpsSingleResult.state, true);
												}
											});
										}
									}
